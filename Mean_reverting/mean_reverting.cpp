#include "mean_reverting.h"
#include "../Database/Database.h"
#include <math.h>
mean_reverting::mean_reverting(const std::string &symb1, const std::string &symb2, const std::string &strt, const std::string &end, int n_years, double x, int past_days)
{
    n_days = past_days;
    max_position = x;
    symbol1 = symb1;
    ...
    data1.generate_data(symbol1, start_date, end_date, n, "mean_reverting");
    data2.generate_data(symbol2, start_date, end_date, n, "mean_reverting");
}
...
void mean_reverting::generate_result()
{
...
void mean_reverting::execute_strategy(const double &x, const double &threshold)
{
    ...
    for (; i < d1.size(); i++)
    {
        ...
        if (z_score > threshold && stocks_inhand > -x)
        {
            ...
            order_statics1.push_back(d1[i].date + ",SELL,1," + std::to_string(d1[i].close));
            order_statics2.push_back(d2[i].date + ",BUY,1," + std::to_string(d2[i].close));
        }
        else if (z_score < -threshold && stocks_inhand < x)
        {
            stocks_inhand++;
            cash_flow -= current_price;
            order_statics1.push_back(d1[i].date + ",BUY,1," + std::to_string(d1[i].close));
            order_statics2.push_back(d2[i].date + ",SELL,1," + std::to_string(d2[i].close));
        }
...
{
    mean_reverting mean_reverting(argv[1], argv[2], argv[6], argv[7], std::stoi(argv[4]), std::stod(argv[3]), std::stoi(argv[4]));
    mean_reverting.execute_strategy(std::stod(argv[3]), std::stod(argv[5]));
    mean_reverting.generate_result();
    return 0;
}